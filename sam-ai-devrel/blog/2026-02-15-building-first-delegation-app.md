...
export default function DelegationDashboard() {
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      ...
      loadDelegations(contract, address);
    } else {
      alert("Please install MetaMask");
    }
  };

  // Load user's delegations
  const loadDelegations = async (contract, address) => {
    const delegationIds = await contract.getUserDelegations(address);
    ...
    setDelegations(perms.map((perm, i) => ({
      id: delegationIds[i],
      ...perm
    })));
  };
  ...
  const grantPermission = async () => {
    ...
    try {
      ...
      loadDelegations(contract, account);
              ...
              <div className="space-y-4">
                {delegations.map((delegation) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => revokePermission(delegation.id)}
...
- **GitHub**: github.com/Samdevrel/delegation-playground
- **Demo**: delegation-playground.vercel.app
