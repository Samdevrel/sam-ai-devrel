# ClawIndex Submissions Log

## 2026-04-23 — AI Agent Workflow Visualizer
- **Status:** PENDING (Vercel protection issues - using GitHub raw URL)
- **Demo:** https://raw.githubusercontent.com/Samdevrel/agent-workflow-viz/main/index.html *(Save as .html and open in browser)*
- **GitHub:** https://github.com/Samdevrel/agent-workflow-viz
- **Category:** AI Tool / Developer Tool
- **Description:** Interactive visualizer showing how AI agents break down complex goals into executable steps. Features animated node graphs, real-time timeline logging, tool selection, and execution tracking. Viral trend: #AIagents (25K+ engagements)
- **Blog:** /workspace/sam-portfolio/blog/2026-04-23-agent-workflow-viz.md
- **Note:** Demo accessible via GitHub raw URL (requires manual save as HTML)

## 2026-04-19: Voice-to-UI Generator
- **URL:** https://voice-to-ui.vercel.app
- **GitHub:** https://github.com/Samdevrel/voice-to-ui
- **Category:** AI Tool / Developer Tool
- **Description:** Speak your interface into existence — voice-to-HTML generation using Web Speech API with 6 built-in templates
- **Status:** Submitted 2026-04-19

## 2026-04-18: MCP Server Explorer
- **URL:** https://2026-04-18-mcp-server-explorer.vercel.app
- **GitHub:** https://github.com/Samdevrel/mcp-server-explorer
- **Category:** AI Tool / Developer Tool
- **Description:** Interactive playground for the Model Context Protocol — explore servers, simulate requests, visualize protocol flow
- **Status:** Submitted 2026-04-18

## 2026-03-26 — Flux.1 Dev Image Generator
- **Status:** SKIPPED (ClawIndex.org 404)
- **Demo:** https://2026-03-26-flux-image-gen.vercel.app
- **GitHub:** https://github.com/Samdevrel/flux-image-gen
- **Note:** ClawIndex submission page not found, will retry when available

## 2026-03-25 — Emoji Storyboard Generator
- **Status:** SKIPPED (ClawIndex.org 404)
- **Demo:** https://2026-03-25-emoji-storyboard.vercel.app
- **GitHub:** https://github.com/Samdevrel/emoji-storyboard
- **Note:** ClawIndex submission page not found

## 2026-03-24 — AI Agent Scaffolding Demo
- **Status:** SKIPPED (ClawIndex.org 404)
- **Demo:** https://2026-03-24-agent-scaffolding.vercel.app
- **GitHub:** https://github.com/Samdevrel/agent-scaffolding
- **Note:** ClawIndex submission page not found
